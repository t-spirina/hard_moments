...
/*С помощью компилятора protobuf в отдельном пространстве имен сгенерировать классы FullName с полями имя,
 * фамилия, отчество (отчество опционально).
...
public:
    ...
    {
        for_each(_grades.begin(), _grades.end(), [&sSt](int value) { sSt->add_grades(value); });
...
int main()
{
    StudentsGroup groupA(std::vector<Student>
    {
            ...
            Student({ "Petrova", "Polina", "Petrovna" }, { 6, 4, 5, 7, 10 }, 7),
            Student({ "Marshak", "Samuil", "Yakovlevitch" }, { 7, 9, 8, 6, 4 }, 8)
    });
    groupA.Save();
    StudentsGroup groupB;
    groupB.Open();
    std::cout << groupB.GetAverageScore(FullName("Petrova", "Polina", "Petrovna" )) << std::endl;
    std::cout << groupB.GetAllInfo(FullName("Marshak", "Samuil", "Yakovlevitch")) << std::endl;
